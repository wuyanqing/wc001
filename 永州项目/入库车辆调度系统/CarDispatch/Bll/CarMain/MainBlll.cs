        ...
        public static DataSet SelectAllCar(string select)
        {
            ...
            try
            {
                switch (select)
                {
                    // 查询出未递交货运单所有的车辆数据
                    case "0":
                        ds = db.ReturnDataSet(visitor, "Selectstate1", "V_Bus_RegistrationInfo");
                        break;
                    //未卸货的车辆总数
                    case "1":
        ...
        public static DataSet SelectAll(string select, string WhereSql)
        {
            ...
            try
            {
                switch (select)
                {
                    //1号伸缩链板机的未卸货排队车辆
                    case "1":
                        ds = db.ReturnDataSet(visitor, "Selectstate2_1", "V_Bus_RegistrationInfo", WhereSql);
                        break;
                    //2号伸缩链板机的未卸货排队车辆
                    case "2":
                        ds = db.ReturnDataSet(visitor, "Selectstate2_2", "V_Bus_RegistrationInfo", WhereSql);
                        break;
                }
            }
            catch (Exception ex)
